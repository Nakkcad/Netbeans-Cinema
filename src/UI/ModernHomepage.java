...
private void addMovieCategory(String categoryTitle, List<Film> films) {
    ...
    SwingUtilities.invokeLater(() -> {
        ...
        // Create horizontal scroll pane with hidden scrollbar
        ...
        // Custom scroll behavior - only horizontal when Shift is pressed
        horizontalScroll.addMouseWheelListener(e -> {
            if (e.isShiftDown()) {
                JScrollBar hBar = horizontalScroll.getHorizontalScrollBar();
                int amount = e.getUnitsToScroll() * hBar.getUnitIncrement() * 3; // Faster scrolling
                hBar.setValue(hBar.getValue() + amount);
                e.consume();
            }
            // Let vertical scroll pass through when Shift isn't pressed
        });
        
        // Also enable horizontal scrolling with mouse drag
        ...
        horizontalScroll.getViewport().addMouseListener(ma);
        horizontalScroll.getViewport().addMouseMotionListener(ma);
